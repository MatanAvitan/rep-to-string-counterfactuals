...
encoder = AutoModel.from_pretrained("sentence-transformers/gtr-t5-base").encoder.cuda()
...
columns_to_store = list(bios_train_df.columns) + ['transformed_hard_text']
transformed_table_wandb = wandb.Table(columns=columns_to_store)

"""
Generate text from repr
"""
log_print('sum((female_mask) & (sequences_length <= MAX_SEQUENCE_LENGTH))')
...
for lower_bound in tqdm.tqdm(range(0, len(female_mask_indices), INVERSION_BATCH_SIZE)):
    ...
    [transformed_table_wandb.add_data(*i) for i
     in bios_train_df.loc[batch_indices, columns_to_store].to_records(index=False).tolist()]
    updated_transformed_table_wandb = wandb.Table(columns=columns_to_store,
                                                  data=transformed_table_wandb.data)
    wandb.log({'transformed_hard_text': updated_transformed_table_wandb})
...
Generate text from repr
"""
log_print('sum((male_mask) & (sequences_length <= MAX_SEQUENCE_LENGTH))')
