...
SUPPORTED_MODELS = ["text-embedding-ada-002", "gtr-base", 'yiyic/t5_me5_base_mtg_en_5m_64', 'gtr-base-64-02-02-24']
...
def load_corrector(embedder: str) -> vec2text.trainers.Corrector:
    ...
    elif embedder == 'gtr-base-64-02-02-24':
        inversion_model = vec2text.models.InversionModel.from_pretrained(
            "/home/nlp/matan_avitan/tmp/checkpoint-106640"
        )
        model = vec2text.models.CorrectorEncoderModel.from_pretrained(
            "jxm/gtr__nq__32__correct"
        )
    elif embedder.startswith('checkpoint'):
        inversion_model = vec2text.models.InversionModel.from_pretrained(
            f"/home/nlp/matan_avitan/tmp/{embedder}"
        )
        model = vec2text.models.CorrectorEncoderModel.from_pretrained(
            "jxm/gtr__nq__32__correct"
        )
    ...
    elif embedder == 'yiyic/t5_me5_base_mtg_en_5m_64':
        inversion_model = vec2text.models.InversionModel.from_pretrained(
            "../models/tmp/t5_me5_base_mtg_en_5m_64_inverter"
        )
        model = vec2text.models.CorrectorEncoderModel.from_pretrained(
            "../models/tmp/t5_me5_base_mtg_en_5m_64_corrector"
        )
    else:
