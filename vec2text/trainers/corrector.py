...
class Corrector(BaseTrainer):
    ...
    def evaluation_loop(
        ...
        """
        self.inversion_trainer.model.to(self.model.device)
        ...
        if not os.path.exists(cache_path):
            ...
            if filter_correct_examples:
                ...
                def embedding_is_not_correct(ex):
                    return (
                        ~torch.isclose(
                            ex["frozen_embeddings"].to(self.model.device),
                            ex["hypothesis_embedding"].to(self.model.device),
                        ).all(dim=1)
                    ).tolist()
    ...
    def generate(
        ...
        # Add beam dimension:
        #       (batch, ...) -> (batch, beam, ...)
        inputs["frozen_embeddings"] = frozen_embeddings.to(self.model.device)
        inputs["hypothesis_input_ids"] = hypothesis_input_ids.to(self.model.device)
        inputs["hypothesis_attention_mask"] = hypothesis_attention_mask.to(self.model.device)
        inputs["hypothesis_embedding"] = hypothesis_embedding.to(self.model.device)
    ...
    def get_frozen_embeddings(
        ...
        return frozen_embeddings.to(self.model.device)
