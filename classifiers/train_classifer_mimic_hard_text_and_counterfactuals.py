...
EXP_NAME = 'ot_hard_text_and_counterfactuals'
MAX_LENGTH = 64
SEED = 0
IS_FIRST = True
...
ot_stacked_w_counterfactuals = pd.concat([ot_stacked['hard_text'],
...
ot_stacked_w_counterfactuals['labels'] = label_encoder.fit_transform(ot_stacked_w_counterfactuals['stacked_p'])
...
train_df = ot_stacked_w_counterfactuals
...
if IS_FIRST:
    ...
    train_ds = train_ds.map(create_input_sequence, batched=True, batch_size=1, remove_columns=columns_to_remove,
                            num_proc=16)
    train_ds.save_to_disk(f'{output_dir}/{EXP_NAME}_train_dataset')
    ...
    validation_ds = validation_ds.map(create_input_sequence, batched=True, batch_size=1,
                                      ...
                                      num_proc=16)
    validation_ds.save_to_disk(f'{output_dir}/{EXP_NAME}_validation_dataset')
else:
    train_ds = datasets.load_from_disk(f'{output_dir}/{EXP_NAME}_train_dataset')
    validation_ds = datasets.load_from_disk(f'{output_dir}/{EXP_NAME}_validation_dataset')
