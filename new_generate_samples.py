...
for i in tqdm.tqdm(range(0, len(bios_train_df), BATCH_SIZE)):
    ...
    encodings.append(get_gtr_embeddings(sents_batch, encoder, tokenizer))
...
"""
female_mask_indices = np.arange(len(bios_train_df))[female_mask]
for lower_bound in tqdm.tqdm(range(0, N_SAMPLES_TO_INVERT, BATCH_SIZE)):
...
Generate text from repr
"""
male_mask_indices = np.arange(len(bios_train_df))[male_mask]
for lower_bound in tqdm.tqdm(range(0, N_SAMPLES_TO_INVERT, BATCH_SIZE)):
...
# bios_train_df.to_csv('bios_data/bios_train_bsw_4_n_steps_20_df.csv', index=False, escapechar='\\')
